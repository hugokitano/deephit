...
def create_FCNet(inputs, num_layers, h_dim, h_fn, o_dim, o_fn, w_init, keep_prob=1.0, w_reg=None):
    '''
        ...
        inputs (tensor)  : input tensor # to hold covariates
        num_layers       : number of layers in FCNet # variable
        h_dim  (int)     : number of hidden units # 'h_dim_shared' 
        h_fn             : activation function for hidden layers (default: tf.nn.relu)
        o_dim  (int)     : number of output units # 'h_dim_shared'
        o_fn             : activation function for output layers (defalut: None)
        w_init           : initialization for weight matrix (defalut: Xavier)
        keep_prob        : keep probabilty [0, 1]  (if None, dropout is not employed) #none?
    '''
    ...
    for layer in range(num_layers): # read this again
        if num_layers == 1:
