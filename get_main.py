...
def f_get_minibatch(mb_size, x, label, time, mask1, mask2):
    ...
    idx = random.sample(idx, mb_size) # take random batch size # from training
...
def get_valid_performance(DATA, MASK, in_parser, out_itr, eval_time=None, MAX_VALUE = -99, OUT_ITERATION=5, seed=1234):
   ...
   #pdb.set_trace()
    ...
    ##### HYPER-PARAMETERS
    mb_size                     = in_parser['mb_size'] #batch size

    iteration                   = in_parser['iteration'] # 50000???

    keep_prob                   = in_parser['keep_prob']
    lr_train                    = in_parser['lr_train'] #what is this
    ...
    parameter_name              = 'a' + str('%02.0f' %(10*alpha)) + 'b' + str('%02.0f' %(10*beta)) + 'c' + str('%02.0f' %(10*gamma))
                                # something like 'a10b01c00'
    initial_W                   = tf.contrib.layers.xavier_initializer() # set initial weights at good position
    ...
    config = tf.ConfigProto() # configure session
    config.gpu_options.allow_growth = True # allow more gpu
    ...
    ### TRAINING-TESTING SPLIT # also validation
                ...
                for t, t_time in enumerate(eval_time):
                    ...
                    if eval_horizon >= num_Category:
                        ...
                        for k in (range(num_Event)):
                            ...
                            va_result2[k, t] = weighted_brier_score(tr_time, (tr_label[:,0] == k+1).astype(int), risk[:,k], va_time, (va_label[:,0] == k+1).astype(int), eval_horizon)
                tmp_valid = np.mean(va_result1[0,:]) # mean over all causes?
                tmp_scores = va_result1[0,:]
                brier_valid = np.mean(va_result2[0,:])


                if tmp_valid >  max_valid:
                    stop_flag = 0
                    max_valid = tmp_valid
                    max_brier = brier_valid
                    max_valid_scores = tmp_scores
                    print( 'updated.... average c-index = ' + str('%.4f' %(tmp_valid)))
                    print( 'scores = ' + np.array_str(max_valid_scores))
