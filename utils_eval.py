...
# The proposed estimators rely on a working model for the conditional censoring distribution-given predictor 
# variables and are based on general theory for censored and missing data [10, 11].
def weighted_c_index(T_train, Y_train, Prediction, T_test, Y_test, Time):
    '''
        ...
        - Time            : time of evaluation (time-horizon when evaluating C-index)
    '''
    G = CensoringProb(Y_train, T_train) # G[0] from 0 to 9218 (times?). G[1] from 1 to 0, not sorted

    N = len(Prediction) # validation
    A = np.zeros((N,N))
    Q = np.zeros((N,N))
    N_t = np.zeros((N,N))
    Num = 0
    ...
    for i in range(N):
        ...
        if len(tmp_idx) == 0:
            ...
            W = (1./G[1, tmp_idx[0]])**2 # weight

        A[i, np.where(T_test[i] < T_test)] = 1. * W # all training times longer than current ith datapoint
        Q[i, np.where(Prediction[i] > Prediction)] = 1. # give weights # all prediction times less than current ith datapoint

        if (T_test[i]<=Time and Y_test[i]==1): # if the time is less than eval horizion and uncensored, set N_t[i] to 1
            N_t[i,:] = 1.      
    Num  = np.sum(((A)*N_t)*Q)
    Den  = np.sum((A)*N_t)
...
def weighted_brier_score(T_train, Y_train, Prediction, T_test, Y_test, Time):
    ...
    Y_tilde = (T_test > Time).astype(float) #if true time is greater than eval horizon
