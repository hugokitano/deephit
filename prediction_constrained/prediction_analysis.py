...
def read_files():
    #pdb.set_trace()
    train_data = np.loadtxt('train_data.txt', delimiter = ",") #3675 x 29
    train_label = np.loadtxt('train_event.txt', delimiter = ",") #3675
    train_time = np.loadtxt('train_time.txt', delimiter = ",") #3675
    train_pred = np.loadtxt('train_pred.txt', delimiter = ",") #3675 x 28

    val_data = np.loadtxt('val_data.txt', delimiter = ",") #919 x 29
    val_label = np.loadtxt('val_event.txt', delimiter = ",") #919
    val_time = np.loadtxt('val_time.txt', delimiter = ",") #919
    val_pred = np.loadtxt('val_pred.txt', delimiter = ",") #919 x 28
    ...
    data_raw  = np.copy(data)  # (5743, 29)
    data = f_get_Normalization(data, 'standard')
    data = np.hstack((ids,data)) # (5743, 30)


    #train
    train_id = np.zeros(train_data.shape[0])
    train_index = np.zeros(train_data.shape[0])
    ...
    train = pd.concat([train_id, is_train, train_data_raw, train_label, train_time, train_pred], axis = 1) #3675, 61    
    ...
    #test_id + is_train + test data + test_label + test_time + test_pred
    #1       + 1        + 29          1          + 1         + 28
    ...
    pdb.set_trace()
    test = pd.concat([test_id, is_train, test_data_raw, test_label, test_time, test_pred], axis = 1)  
    ...
    tofile = tofile.sort_values(by=['id'])
    tofile.reset_index(inplace=True)
    tofile.drop("index",axis=1,inplace=True)
