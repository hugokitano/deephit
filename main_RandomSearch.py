...
def get_random_hyperparameters(out_path):
    ...
    SET_LAYERS        = [1,2] #number of layers
    SET_NODES         = [25, 50, 75, 100] #number of nodes

    SET_ACTIVATION_FN = ['elu'] #non-linear activation functions

    SET_ALPHA         = [0.1, 0.5, 1.0, 3.0, 5.0] #alpha values -> log-likelihood loss 
    SET_BETA          = [0.0, 0.1] #beta values -> ranking loss
    SET_GAMMA         = [0.1, 0.5, 1.0, 3.0, 5.0] #gamma values -> calibration loss


    new_parser = {'mb_size': SET_BATCH_SIZE[np.random.randint(len(SET_BATCH_SIZE))], #3 
                 ...
                 'h_dim_shared': SET_NODES[np.random.randint(len(SET_NODES))], #4
                 'h_dim_CS': SET_NODES[np.random.randint(len(SET_NODES))], # cause-specific # 4
                 'num_layers_shared':SET_LAYERS[np.random.randint(len(SET_LAYERS))], # 2
                 'num_layers_CS':SET_LAYERS[np.random.randint(len(SET_LAYERS))], # cause-specific # 2
                 'active_fn': SET_ACTIVATION_FN[np.random.randint(len(SET_ACTIVATION_FN))], # 1

                 'alpha':1.0, #default (set alpha = 1.0 and change beta and gamma)
                 'beta':SET_BETA[np.random.randint(len(SET_BETA))], # 2
...
def random_hyperparameter_string(new_parser):
    str_mb_size = "mb_size:" + str(new_parser['mb_size']) + ","
    str_h_dim_shared = "h_dim_shared:" + str(new_parser['h_dim_shared']) + ","
    str_h_dim_CS = "h_dim_CS:" + str(new_parser['h_dim_CS']) + ","
    str_num_layers_shared = "num_layers_shared:" + str(new_parser['num_layers_shared']) + ","
    str_num_layers_CS = "num_layers_CS:" + str(new_parser['num_layers_CS']) + ","
    str_active_fn = "active_fn:" + str(new_parser['active_fn']) + ","
    str_beta = "beta:" + str(new_parser['beta'])
    string = str_mb_size + str_h_dim_shared + str_h_dim_CS + str_num_layers_shared + str_num_layers_CS + str_active_fn + str_beta
    return string

def write_hyperparameters_to_file(dictionary, data_mode):
    csv_file = data_mode + '/' + 'hyperparameters.csv'
    with open(csv_file, 'w') as f:
        for key in dictionary.keys():
            scores = str(dictionary[key])
            scores = scores[1:-1]
            f.write("%s,%s\n"%(key,scores))
...
data_mode                   = 'MEC' ## change the dataset
seed                        = 1234
...
for itr in range(OUT_ITERATION):
    ...
    for r_itr in range(RS_ITERATION):
        ...
        print(new_parser)
        hyperparameter_string = random_hyperparameter_string(new_parser) 

        # get validation performance given the hyperparameters
        tmp_max, tmp_brier = get_main.get_valid_performance(DATA, MASK, new_parser, itr, EVAL_TIMES, MAX_VALUE=max_valid)
        hyperparameter_dict[hyperparameter_string] = (tmp_max, tmp_brier)
        ...
        if ((r_itr + 1) % 10) == 0:
            write_hyperparameters_to_file(hyperparameter_dict, data_mode)
