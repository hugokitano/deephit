...
LOSS FUNCTIONS:
    - 1. loglikelihood (this includes log-likelihood of subjects who are censored) L1
    - 2. rankding loss (this is calculated only for acceptable pairs; see the paper for the definition) L2
...
class Model_DeepHit:
    def __init__(self, sess, name, input_dims, network_settings):
        ...
        self.h_dim_CS           = network_settings['h_dim_CS'] # cause-specific
        self.num_layers_shared  = network_settings['num_layers_shared']
        self.num_layers_CS      = network_settings['num_layers_CS'] # cause-specific
    ...
    def _build_net(self):
        with tf.variable_scope(self.name):
            ...
            #at least three FC_Nets
            #l2 regularizer for weight regularizer
            # note the parameters are duplicated
            ...
            last_x = self.x  #for residual connection # i thought dropped?

            h = tf.concat([last_x, shared_out], axis=1) # data + output of fc_net
            ...
            for _ in range(self.num_Event):
                ...
                out.append(cs_out)
            out = tf.stack(out, axis=1) # stack referenced on subject # layer columns on listed tensors
            out = tf.reshape(out, [-1, self.num_Event*self.h_dim_CS]) # stack causes
            out = tf.nn.dropout(out, keep_prob=self.keep_prob)

            #final softmax layer. notice it uses FC_Net itself
            ...
            self.out = tf.reshape(out, [-1, self.num_Event, self.num_Category]) # num_Event * num_Category split out to different causes
