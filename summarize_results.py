...
for out_itr in range(OUT_ITERATION):
    ...
    pred_train = model.predict(tr_data)
    pred_val = model.predict(va_data)
    prediction_directory = "prediction_constrained/"
    if out_itr == 4: # change
        np.savetxt(prediction_directory + 'test_pred.txt', pred[:,0,:], delimiter = ",")
        np.savetxt(prediction_directory + 'test_data.txt', te_data, delimiter = ",")
        np.savetxt(prediction_directory + 'test_event.txt', te_label, delimiter = ",")
        np.savetxt(prediction_directory + 'test_time.txt', te_time, delimiter = ",")

        np.savetxt(prediction_directory + 'train_pred.txt', pred_train[:,0,:], delimiter = ",")
        np.savetxt(prediction_directory + 'train_data.txt', tr_data, delimiter = ",")
        np.savetxt(prediction_directory + 'train_event.txt', tr_label, delimiter = ",")
        np.savetxt(prediction_directory + 'train_time.txt', tr_time, delimiter = ",")

        np.savetxt(prediction_directory + 'val_pred.txt', pred_val[:,0,:], delimiter = ",")
        np.savetxt(prediction_directory + 'val_data.txt', va_data, delimiter = ",")
        np.savetxt(prediction_directory + 'val_event.txt', va_label, delimiter = ",")
        np.savetxt(prediction_directory + 'val_time.txt', va_time, delimiter = ",")
    ...
    for t, t_time in enumerate(EVAL_TIMES):
        ...
        if eval_horizon >= num_Category:
            ...
            risk_val = np.sum(pred_val[:,:,:(eval_horizon+1)], axis=2)
            for k in range(num_Event):
                ...
                va_result = weighted_c_index(tr_time, (tr_label[:,0] == k+1).astype(int), risk_val[:,k], va_time, (va_label[:,0] == k+1).astype(int), eval_horizon)
