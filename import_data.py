...
def f_get_fc_mask3(time, meas_time, num_Category):
    ...
    else:                    #single measurement for both metabric and synthetic
        for i in range(np.shape(time)[0]):
...
def import_dataset_SYNTHETIC(norm_mode='standard'):
    ...
    #this dataset has time of event, event, and covariates all in one csv. what is true time, true label? 
    # rue label seems to = label except when label = 0 (censored)
    ...
    num_Category    = int(np.max(time) * 1.2)  #to have enough time-horizon, # num days
...
def import_dataset_METABRIC(norm_mode='standard'):
    ...
    df1 = pd.read_csv(in_filename1, sep =',') # covariates
    df2 = pd.read_csv(in_filename2, sep =',') # event-time and label

    data  = np.asarray(df1)
    data  = f_get_Normalization(data, norm_mode) #normalized covariates
...
def import_dataset_MEC(norm_mode='standard'):
    # pdb.set_trace()
...
def import_dataset_general(in_filename, norm_mode='standard'):
    df = pd.read_csv(in_filename, sep =',')
    df = df.drop(df.columns[0],axis=1) # if necessary
    ...
    DATA            = (data, time, label)
    MASK            = (mask1, mask2)
    DATA_RAW        = (ids, data_raw)

    return DIM, DATA, MASK, DATA_RAW
